...
export default function PaletteCard({ item, isActive, isFavorite, onApply, onToggleFavorite }: Props) {
  const swatches = [
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <button
          className={`rounded-lg border px-3 py-1 text-[11px] font-semibold transition ${
            isFavorite
              ? 'border-red-400/60 text-red-200 hover:border-red-300'
              : 'border-white/15 text-muted hover:border-accent/40 hover:text-white'
          }`}
          type="button"
          onClick={(e) => {
            e.stopPropagation()
